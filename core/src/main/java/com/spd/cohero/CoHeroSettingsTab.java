...
public final class CoHeroSettingsTab extends Component {
        ...
        if (companion != null && companion.isAlive()) {
            ...
            enemySpawnSlider = new OptionSlider(
                ...
                protected void onChange() {
                    requireCompanion().setEnemySpawnMultiplierQuarters(getSelectedValue());
            ...
            debugLog = new CheckBox(CoHeroMessages.get("inventory.debug_log")) {
                ...
                protected void onClick() {
                    super.onClick();
                    requireCompanion().setDebugLogEnabled(checked());
                }
            };
    ...
    private String enemySpawnValueText() {
        return String.format(
                Locale.ENGLISH,
                "%.2fx",
                requireCompanion().enemySpawnMultiplierQuarters() / 4f);
    }

    private CoHeroAlly requireCompanion() {
        if (companion == null || !companion.isAlive()) {
            throw new IllegalStateException("CoHero settings require an active companion");
        }
        return companion;
    }
}
