...
2. **有近戰武器時**
   ...
   - 當敵人已進入該近戰武器的合法攻擊距離時，普通情況使用近戰；但高閃避且可被有效傷害法杖作用的目標例外，會優先法杖。對純近戰敵人若仍有至少一格空隙且存在合法遠攻，則不會為了更高的近戰平均傷害主動貼近；若目標正在從非相鄰距離施加遠程壓力，延伸近戰射程也不代表理想交戰距離，AI 仍依 anti-ranged 規則處理。
