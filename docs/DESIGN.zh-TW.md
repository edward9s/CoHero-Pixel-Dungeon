...
6. 把風區以 live runtime exits 做 room-internal path distance 分割：`outsideRoom` 每個可通行格比較「到 Hero 房共用門」與「到其他目前可通行出口」的距離；只有前者嚴格較短的格子屬於 guard area，等距中線不算。**Hero 房本身與共用門格永遠不屬於 guard area**，共用門只作為 Hero 房與 outside room 之間的 transit cell，不能成為 `guard_roam` 目標。若沒有其他可通行出口，則使用該 Room 內所有符合上述邊界的可達格。
