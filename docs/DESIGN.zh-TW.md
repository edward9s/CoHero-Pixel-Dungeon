...
4. **有法杖時**
   ...
   - 哨衛法杖建立的 ward 會標記為 Hero-owned 或 CoHero-owned，兩邊分開計算原版 ward energy 上限，因此不會互相吃掉法杖提供的 ward 配額。額度仍是硬限制；CoHero 不會遠端刪除 ward 作弊，Hero 仍可依原版互動手動解除 CoHero 的 ward 來騰出 energy。
   ...
   - 新 ward 另外要求首次偵測機率至少 50%，並用敵人移動後仍在 ward 視野／彈道中的位置數作為主要評分；因此不再只看「現在射不射得到」，而會偏好能覆蓋敵人下一步、同時不會被先手拆掉的位置。tier 3→4 首次取得實際耐久會提高升級價值，但升級既有 ward 不再無條件優先於新放置。睡眠敵人與環境危險的既有安全檢查仍保留。
