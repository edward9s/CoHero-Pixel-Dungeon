...
9. 「在哪裡把風」與「在哪裡交戰」是兩個獨立問題。特殊交戰目標由 `CoHeroCombatObjectiveController` 單獨負責，優先級位於 hazard／生存逃跑之下、普通攻擊與 guard 之上；controller 只限制可主動攻擊的敵人與誘敵站位，不重做既有近戰、投擲、法杖或生存系統。第一個 objective 是獻祭之火：Hero 位於仍有有效 `SacrificialFire` blob 的房間時，只建立可用 objective 與祭火格 `NEIGHBOURS9` engagement zone，狀態仍是 `idle`，完全不改變普通 guard。只有 CoHero 實際看見可攻擊敵人且敵人仍在 engagement zone 外，才考慮由 `idle → luring`；但若任何當前可見威脅已能從非相鄰位置直接攻擊 Hero 或 CoHero（沿用 Mob 的 `coHeroCanAttackFrom()` capability，而不是遠程怪類別清單），就不啟用 lure，直接交回普通戰鬥，因為這類敵人站在門口即可對室內施壓。只有其餘近戰型威脅才會取消門口交戰、保存 zone 內安全 staging cell 並向祭壇側後撤。`luring` 完全由當前視野驅動；一旦沒有任何可見清醒敵人，立即 `luring → idle`，同回合恢復普通 support／guard，不在祭壇區繼續乾等，讓敵人有機會重新看到 CoHero／Hero 而追上來。敵人再次出現且仍在 zone 外時再重新進入 lure。敵人一踏入 zone，該回合轉成 `engage`，普通 combat selector 只看 zone 內敵人；下一回合若敵人仍在 zone 就繼續正常交戰，若又退出則重新進入 lure。若誘敵路徑完全不可行而 CoHero 已正在受到直接攻擊，該回合允許 fail-open 回普通戰鬥，避免 AI 因 objective 死鎖。objective 是否存在只看目前 live gameplay state（例如 blob），不依賴 `SacrificeRoom.class`。
