...
- Hero 與 CoHero 不共享遊戲規則層的 FOV：`Dungeon.level.heroFOV` 始終只代表玩家 Hero 視野。CoHero 另外維持自己的 `fieldOfView`，只在畫面呈現層合併，用來顯示 CoHero 周圍地形與角色，不讓 CoHero 遠端戰鬥影響 Hero 的行動、技能或其他依賴 `heroFOV` 的原版規則。這個 render visibility union **不得**拿來決定 gameplay，但可以決定 presentation：CoHero 的移動、近戰、投擲／靈能弓與 Wand gameplay 永遠先依 SPD Actor 時間軸序列結算；只要來源或目標位於 Hero + CoHero 的聯集可見範圍，就播放非阻塞 presentation，只有完全不在聯集視野時才省略。Presentation 再分成 foreground / background：CoHero 移動永遠是 background；攻擊／投擲／法杖若來源或目標位於 `heroFOV` 才是 foreground，只有 CoHero FOV 看得到時仍完整播放但屬於 background。Background presentation 絕不阻止 Hero ready；foreground presentation 才允許在 Actor queue 回到 Hero 時暫緩輸入，且只等同批 foreground 中最慢的一個。若同一個 CoHero 在自己的上一個 presentation 尚未結束前再次取得回合，仍只等待自己的 presentation，避免 sprite callback 被下一個動作覆蓋。CoHero movement 不再呼叫只認 `heroFOV` 的原版 `Char.moveSprite()`；在聯集視野內直接以 `sprite.move()` 播放，完全不可見時才直接 `place()`。放大鏡／右鍵檢查維持獨立語意：若某個 actor 位於 CoHero 當前 FOV，即使不在 Hero FOV，也允許檢視該 actor 的 info；地板、物品、陷阱等仍只依 Hero FOV。CoHero 的基礎 `viewDistance` 每次進層與行動前直接同步 `Dungeon.level.viewDistance`，所以「沒入黑暗」、DARK feeling、特殊 Boss 關卡或其他上游樓層視距調整都會同樣影響 CoHero；若 CoHero 具有原版 `Light` buff，則依原版規則至少提升至 6 格。
...
因此採用 fail-closed capability adapter：Wand 保留原版本身的效果，但 CoHero cast 一律先完成必要 targeting／AOE 準備、`onZap()` 與 charge 結算；只要 caster 或 target 位於 Hero + CoHero 聯集視野，就建立 presentation-only FX，完全不可見時才省略。若 caster／target 只在 CoHero FOV，該 FX 為 background，不阻止 Hero ready；若 Hero 本人也看得到，才列為 foreground。FX callback 只用來結束 `CoHeroPresentation` pending，不再推進 gameplay 或 Actor 時間。CoHero adapter 只負責判斷 targeting、安全性與「直接傷害／控制／逃生／支援」語意。未知 Wand 類型不猜測、不自動使用。
