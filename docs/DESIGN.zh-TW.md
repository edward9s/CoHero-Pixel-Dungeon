...
9. 「在哪裡把風」與「在哪裡交戰」是兩個獨立問題。特殊交戰目標由 `CoHeroCombatObjectiveController` 單獨負責，優先級位於 hazard／生存逃跑之下、普通攻擊與 guard 之上；controller 只限制可主動攻擊的敵人與誘敵站位，不重做既有近戰、投擲、法杖或生存系統。第一個 objective 是獻祭之火：只要 Hero 位於仍有有效 `SacrificialFire` blob 的房間，就以實際祭火格及其 `NEIGHBOURS9` 建立 engagement zone。敵人尚未進 zone 時，CoHero 會在 zone 內選擇安全、受攻擊較少且靠近祭火的站位等待／後撤，不在門口主動交戰；敵人進 zone 後才交回普通戰鬥 selector。沒有可見敵人時也先在 objective 內就位，因此一般 guard 不會先把 CoHero 送到門口。若誘敵路徑完全不可行而 CoHero 已正在受到直接攻擊，該回合允許 fail-open 回普通戰鬥，避免 AI 因 objective 死鎖。objective 是否存在只看目前 live gameplay state（例如 blob），不依賴 `SacrificeRoom.class`。
