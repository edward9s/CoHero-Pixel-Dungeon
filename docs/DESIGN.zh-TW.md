...
CoHero 背包視窗頂部固定顯示目前即時基本數值：Lv、HP（有護盾時顯示為 `HP+shield/HT`）、STR、近戰 DMG 範圍、DR 範圍與實際 SPD 倍率。這些值直接由 CoHero 當前裝備、固有 trait、戒指與負重計算，不保存第二份 UI 專用數值。視窗維持單欄與 5 欄背包格，透過縮小格子間距、在窄螢幕時微縮 slot，以及縮窄敵人生成倍率 slider 與「從玩家背包加入物品」按鈕來適應不同手機寬度；不因橫屏改成 4 欄或雙欄，以避免增加視窗高度。
