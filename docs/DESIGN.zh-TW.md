...
7. `guardRoom` 不會在發現單出口房時立刻成立。CoHero 必須先真正進入 `guardArea`，才正式啟用把風。正式到位前，若 CoHero 還在樓層其他地方，就用 `followHero()` 往 Hero 方向靠近；若已和 Hero 同處單出口房（例如一起在煉金房），則明確前往最近的可達 `guardArea` 格。把風一旦成立，只要 Hero 仍位於原 Hero 房的矩形範圍內（包含共用門／邊界格），guard state 就持續有效；不因 `RegularLevel.room(hero.pos)` 在門格回傳 `null` 而清除。CoHero 若位於 `guardArea ∪ Hero 房` 之外，`support Hero` 會直接高於 loot recovery、普通探索與 roaming，直到回到合法區域。若 CoHero 已進 Hero 房且 Hero 附近仍有支援威脅，普通戰術位移只能留在 Hero 房內，不得因投擲射線、掩體或 melee positioning 又退回門外；只有 hazard escape、combat survival retreat、無敵敵人脫離等真正生存性移動可破例離房。普通漫遊、`getCloser()` 尋路與最後的 `move()` 仍共同受 guard domain 約束。Hero 真正離開原 Hero 房後才解除 guard state。第 4 點的 4/8 格回援規則永遠優先於把風。
