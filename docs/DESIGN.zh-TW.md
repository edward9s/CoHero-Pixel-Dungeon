...
5. 在具有原版 `Room` topology 的 `RegularLevel`，若 Hero 位於拓樸上只有一個連接（`room.connected.size() == 1`）的非入口／非出口房間，而且沒有第 4 點的附近敵人需要支援，CoHero 進入把風準備。先取得 Hero 房唯一直接連出去的 `outsideRoom`，但不再把整個 Room 都當成把風範圍；Hero 房與 `outsideRoom` 共用的門是把風區的入口錨點。
6. 把風範圍以 room-internal path distance 做分區：`outsideRoom` 每個可通行格都比較「到 Hero 這扇門的距離」與「到其他目前可通行出口的距離」；只有前者嚴格較短的格子才屬於 `guardArea`，距離相等的中線不算。這使長直走廊自然只保留 Hero 入口一側約半段，多出口 ConnectionRoom 則只保留 Hero 入口對應的那一瓣，而不是整個生成用 Room。若沒有其他目前可通行出口，`guardArea` 才等同該 Room 內所有可達格。
7. `guardRoom` 不會在發現單出口房時立刻成立。CoHero 必須先真正進入 `guardArea`，才正式啟用把風。正式到位前，若 CoHero 還在樓層其他地方，就用 `followHero()` 往 Hero 方向靠近；若已和 Hero 同處單出口房（例如一起在煉金房），則明確前往最近的可達 `guardArea` 格。把風一旦成立，只要 Hero 仍位於原 Hero 房的矩形範圍內（包含共用門／邊界格），guard state 就持續有效；不因 `RegularLevel.room(hero.pos)` 在門格回傳 `null` 而清除。CoHero 若位於 `guardArea ∪ Hero 房` 之外，`support Hero` 會直接高於 loot recovery、普通探索與 roaming，直到回到合法區域。除此之外，loot recovery／普通探索之前還有獨立的 exploration gate：每次要進入這一層行為時都重新檢查 Hero 是否仍位於既有 `guardHeroRoom`，或目前位於任一非入口／非出口的單連接 Room；只要成立，就禁止 loot recovery 與 explore，fallback 為 `followHero()`，並輸出 `guard_explore_gate`。若 CoHero 已進 Hero 房且 Hero 附近仍有支援威脅，普通戰術位移只能留在 Hero 房內，不得因投擲射線、掩體或 melee positioning 又退回門外；只有 hazard escape、combat survival retreat、無敵敵人脫離等真正生存性移動可破例離房。普通漫遊、`getCloser()` 尋路與最後的 `move()` 仍共同受 guard domain 約束。Hero 真正離開原 Hero 房後才解除 guard state。第 4 點的 4/8 格回援規則永遠優先於把風。
