...
6. 把風區不再用固定距離，也不再把 `outsideRoom` 依出口距離切半。系統從 Hero 房唯一出口外側的第一個可通行格開始，直接沿目前 `level.passable[]` 的四向拓樸向外追蹤：只要當前位置扣除來路後仍只有唯一前進方向，就持續把該格納入 guard area；遇到第一個分岔、開放區、死路或 `outsideRoom` 的另一個 runtime exit 就停止。因此直通道多長就可守多長，不需要 3 格之類的 magic number；一進開放房或岔路則自然停在拓樸決策點。Hero 房本身、共用門格與另一端出口都不屬於 guard area，共用門只作 transit。
