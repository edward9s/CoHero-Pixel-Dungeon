...
- 無敵敵人不納入可攻擊目標：判斷沿用 SPD 的 `mob.isInvulnerable(CoHeroAlly.class)` 語意，而不是只看某個 buff 名稱。若同時還有可傷害的可見敵人，CoHero 會改打那些目標；若目前可見威脅全部都對 CoHero 無敵，則停止一切近戰／投擲／法杖／setup，優先拉開距離。一般移動無法逃開時，依序嘗試 Blink、Teleportation、Invisibility，最後才用立即生存資源撐住，不會對無敵目標浪費攻擊。
