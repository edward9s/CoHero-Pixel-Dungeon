...
7. `guardRoom` 不會在發現單出口房時立刻成立。CoHero 必須先真正進入 `guardArea`，才正式啟用把風。正式到位前，若 CoHero 還在樓層其他地方，就用 `followHero()` 往 Hero 方向靠近；若已和 Hero 同處單出口房（例如一起在煉金房），則明確前往最近的可達 `guardArea` 格。啟用後，普通漫遊、`getCloser()` 尋路與最後的 `move()` 都共同受 `guardArea ∪ Hero 房` 約束：CoHero 可以進 Hero 房支援，但不能越過 guardArea 的拓樸中線跑向其他出口。支援結束而 CoHero 位於 Hero 房內時，會再次回到 `guardArea`。若傳送等非步行效果把 CoHero 移到兩者之外，則取消既有 guard 狀態重新集合。Hero 離開單出口房或拓樸配對改變時也會清除 guard state。第 4 點的 4/8 格回援規則永遠優先於把風。
