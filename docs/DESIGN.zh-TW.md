...
CoHero 背包視窗頂部固定顯示目前即時基本數值：Lv、HP（有護盾時顯示為 `HP+shield/HT`）、STR、近戰 DMG 範圍、DR 範圍與實際 SPD 倍率。這些值直接由 CoHero 當前裝備、固有 trait、戒指與負重計算，不保存第二份 UI 專用數值。視窗採 responsive layout：直向依目前 UI 可用寬度縮窄並在需要時把背包降為 4 欄；橫向改為左右雙欄，左側放數值、生成率與裝備控制，右側放背包，避免固定寬度或縱向高度超出手機畫面。
