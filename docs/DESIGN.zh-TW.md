...
- 戒指效果與戒指數值顯示都以實際 `RingBuff.target` 為持有者。原版盜賊的 `EnhancedRings` 只在戒指 buff 實際掛在 Hero 身上時提供 +1；Hero 吃豐饒角等 Artifact 觸發的暫時戒指強化，不會外溢到 CoHero 自己裝備的 Haste、Force 或其他戒指，也不會只在 UI 說明中錯誤顯示 +1。
