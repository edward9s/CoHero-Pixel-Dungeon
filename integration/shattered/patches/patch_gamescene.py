...
tag_bounds_methods = (
    "\t}\n\n"
    "\tpublic static RectF coHeroOverlappingTagBounds(RectF probe) {\n"
    "\t\tif (scene == null || probe == null) return null;\n\n"
    "\t\tRectF bounds = null;\n"
    "\t\tfor (Gizmo member : scene.members) {\n"
    "\t\t\tif (!(member instanceof Tag)) continue;\n\n"
    "\t\t\tTag tag = (Tag) member;\n"
    "\t\t\tif (!tag.exists || !tag.isVisible() || !tag.isActive() || tag.camera() != uiCamera) continue;\n\n"
    "\t\t\tRectF rect = new RectF(tag.left(), tag.top(), tag.right(), tag.bottom());\n"
    "\t\t\tif (probe.intersect(rect).isEmpty()) continue;\n"
